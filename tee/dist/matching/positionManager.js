 ...
 */
import { encodePositionKey, getUserPositionsPrefix, } from './priceEncoder.js';
...
export class PositionManager {
     ...
     */
    async getPosition(userId, marketId, outcomeIndex) {
     ...
     */
    async updatePosition(userId, marketId, outcomeIndex, shareDelta, fillPrice) {
        const key = encodePositionKey(userId, marketId, outcomeIndex);
        ...
        if ((currentShares > 0n && shareDelta < 0n) ||
            ...
            const priceMultiplier = 1000000n; // 6 decimal precision
        ...
        else if ((currentShares >= 0n && shareDelta > 0n) ||
            (currentShares <= 0n && shareDelta < 0n)) {
     ...
     */
    async settleMarket(marketId, winningOutcome, outcomeCount) {
        const results = [];
     ...
     */
    async getUserMarketExposure(userId, marketId) {
     ...
     */
    async deserializePosition(data) {
        try {
 ...
 */
export function createPositionManager(pmtState) {
