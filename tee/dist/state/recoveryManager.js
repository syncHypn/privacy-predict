...
export class RecoveryManager {
    ...
    contractCaller;
    eventListener;
    constructor(stateManager, contractCaller, eventListener) {
    ...
    async recover() {
        ...
        try {
            ...
            const isConsistent = await this.verifyStateConsistency(currentRoot, state.onChainRoot);
            if (!isConsistent) {
                ...
                const fromBlock = await this.getBlockForVersion(state.snapshotVersion ?? 0);
     ...
     */
    async verifyStateConsistency(localRoot, onChainRoot) {
     ...
     */
    async getBlockForVersion(version) {
     ...
     */
    async replayMissedEvents(fromBlock) {
 ...
 */
export function createRecoveryManager(stateManager, contractCaller, eventListener) {
