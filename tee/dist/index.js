...
class TEEApplication {
    ...
    async initialize() {
        ...
        this.stateManager = createStateManager(this.pmtState, this.keyManager, ipfsClient, config.snapshotIntervalMatches);
        // Initialize PMT state
        await this.pmtState.initialize();
        // Initialize blockchain components
        const teeSignerPrivateKey = process.env['TEE_SIGNER_PRIVATE_KEY'];
        if (!teeSignerPrivateKey) {
            throw new Error('TEE_SIGNER_PRIVATE_KEY environment variable required');
        }
        this.contractCaller = createContractCaller(this.providerManager, config.contracts, teeSignerPrivateKey);
        this.eventListener = createEventListener(this.providerManager, config.contracts, config.pollingIntervalMs);
        // Initialize recovery manager
        this.recoveryManager = createRecoveryManager(this.stateManager, this.contractCaller, this.eventListener);
        ...
        this.matcher = createMatcher(this.orderBook, this.positionManager, this.stateManager);
        // Initialize oracle client
        this.oracleClient = createOracleClient(this.positionManager, this.stateManager, this.contractCaller);
    ...
    async handleOrderSubmitted(event) {
        ...
        try {
            ...
            const payload = decryptOrderInTEE(Buffer.from(encryptedData).toString('base64'), userPublicKey, this.keyManager.getSecretKey());
            // Process the order
            const result = await this.matcher.processOrder(payload, event.orderId, event.user, event.marketId, Number(event.timestamp));
            // Commit root to chain
            const attestation = await this.keyManager.generateAttestation();
            await this.contractCaller.commitRoot(result.newRoot, result.matchId, attestation);
    ...
    async handleOrderCancelled(event) {
        try {
            ...
            if (success) {
                // Commit new state
                const attestation = await this.keyManager.generateAttestation();
                await this.contractCaller.commitRoot(this.stateManager.getRoot(), event.orderId, // Use orderId as matchId for cancellations
                attestation);
            }
            await this.stateManager.setLastProcessedBlock(event.blockNumber);
     ...
     */
    async handleWithdrawalRequested(event) {
        try {
            ...
            await this.contractCaller.processWithdrawal(event.user, balance.available, proof);
            // Update balance
    ...
    async handleMarketResolved(event) {
        try {
            ...
            if (update.users.length > 0) {
                await this.contractCaller.batchUpdateBalances(update.users, update.encryptedBalances, update.stateRoot);
            }
            // Commit new root
            const attestation = await this.keyManager.generateAttestation();
            await this.contractCaller.commitRoot(this.stateManager.getRoot(), event.marketId, attestation);
            await this.stateManager.setLastProcessedBlock(event.blockNumber);
