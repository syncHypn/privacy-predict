...
const ORDER_SUBMITTED_ABI = parseAbiItem(
...
const WITHDRAWAL_REQUESTED_ABI = parseAbiItem(
  'event WithdrawalRequested(address indexed user, bytes32 indexed commitmentHash)'
);

const MARKET_RESOLVED_ABI = parseAbiItem(
...
export class EventListener {
   ...
   */
  private async fetchOrderSubmittedEvents(fromBlock: bigint, toBlock: bigint): Promise<void> {
    if (!this.handlers.onOrderSubmitted) return;
   ...
   */
  private parseOrderSubmittedLog(log: Log<bigint, number, false>): OrderSubmittedEvent | null {
    try {
      const topics = log.topics as [Hex, Hex, Hex, Hex];
      const orderId = topics[1];
      const marketId = topics[2];
      const user = `0x${topics[3]?.slice(26)}` as Address;
      ...
      const encryptedPayload = ('0x' + data.slice(194, 194 + payloadLength * 2)) as Hex;

      if (!orderId || !marketId) return null;
   ...
   */
  private async fetchOrderCancelledEvents(fromBlock: bigint, toBlock: bigint): Promise<void> {
    if (!this.handlers.onOrderCancelled) return;
   ...
   */
  private parseOrderCancelledLog(log: Log<bigint, number, false>): OrderCancelledEvent | null {
    try {
      const topics = log.topics as [Hex, Hex, Hex];
      const orderId = topics[1];
      const user = `0x${topics[2]?.slice(26)}` as Address;
   ...
   */
  private async fetchDepositEvents(fromBlock: bigint, toBlock: bigint): Promise<void> {
    if (!this.handlers.onDeposit) return;
   ...
   * Parse Deposit log
   */
  private parseDepositLog(log: Log<bigint, number, false>): DepositEvent | null {
    try {
      const topics = log.topics as [Hex, Hex, Hex];
      const user = `0x${topics[1]?.slice(26)}` as Address;
   ...
   */
  private async fetchWithdrawalRequestedEvents(fromBlock: bigint, toBlock: bigint): Promise<void> {
    if (!this.handlers.onWithdrawalRequested) return;
   ...
   */
  private parseWithdrawalRequestedLog(log: Log<bigint, number, false>): WithdrawalRequestedEvent | null {
    try {
      const topics = log.topics as [Hex, Hex, Hex];
      const user = `0x${topics[1]?.slice(26)}` as Address;
   ...
   */
  private async fetchMarketResolvedEvents(fromBlock: bigint, toBlock: bigint): Promise<void> {
    if (!this.handlers.onMarketResolved) return;
   ...
   */
  private parseMarketResolvedLog(log: Log<bigint, number, false>): MarketResolvedEvent | null {
    try {
