...
export class ProviderManager {
  private publicClients: PublicClient[] = [];
  ...
  async checkAllHealth(): Promise<void> {
    await Promise.all(
      this.publicClients.map((_, index) => this.checkHealth(index))
    );
  }
   ...
   */
  private getBestProviderIndex(): number {
