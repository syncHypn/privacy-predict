...
import type { OrderPayload, OrderValue, Fill, MatchResult } from '../types.js';
import type { PMTOrderBook } from './orderBook.js';
import type { PositionManager } from './positionManager.js';
import type { StateManager } from '../state/stateManager.js';
import { orderSideToKeyString } from './priceEncoder.js';
import {
...
export class Matcher {
  constructor(
    private orderBook: PMTOrderBook,
  ...
  async processOrder(
    ...
    const matchSide: 'bid' | 'ask' = payload.side === 'BUY' ? 'ask' : 'bid';
    ...
    for await (const makerOrder of this.orderBook.iterateMatchableOrders(
      marketId,
      ...
      // Execute fill
      const fill = await this.executeFill(
        makerOrder,
        orderId,
        userId,
        fillAmount,
      ...
      if (newMakerAmount === 0n) {
        ...
        const makerUnlock = this.calculateRequiredCollateral(
          makerOrder.side,
          makerOrder.price,
          0n
        );
    ...
    if (remainingAmount > 0n) {
      ...
      const filledCollateral = this.calculateRequiredCollateral(
        payload.side,
        payload.price,
        payload.amount - remainingAmount
      );
    ...
    const matchId = keccak256(
      toHex(`${orderId}:${timestamp}:${fills.length}`)
    ) as Hex;
   ...
   */
  private validateOrder(payload: OrderPayload): ValidationResult {
    // Check side
   ...
   * Cancel an order
   */
  async cancelOrder(orderId: Hex, userId: Address): Promise<boolean> {
    ...
    const collateral = this.calculateRequiredCollateral(
      order.side,
      order.price,
      order.amount
    );
